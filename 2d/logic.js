...
class Util {
    ...
    // Функция копирования для timeline... arr.splice не работает
...
class Model {
    ...
    createFirstPopulation() {
        ...
        this.populations[this.getBestRes(this.populations, "Index")].makeBest();

        this.model_state.time_line.push(Util.copyPopulations(this.populations));
    }

    // Функция для возвращения лучшего результата в массиве генов
    // return Gene, если type == Object
    // return best f(x) in array, если type == f(x)
    // return index in array, в других случаях
    getBestRes(genArray, type){
        let max = Util.f(genArray[0]), res_index=0, res_obj = genArray[0];
        for (let i = 1; i < genArray.length; i++) {
            let t = Util.f(genArray[i]);
            if (t > max) {
                max = t;
                res_index = i;
                res_obj = genArray[i];
    ...
    process() {
        ...
        let new_best_result = prev_best_result = this.getBestRes(this.populations, "f(x)");
        ...
        do {
            ...
            for (let j = 0; j < len; j++) {
                ...
                let parent_a = this.getBestRes(stack, "Object");
                stack.splice(stack.indexOf(parent_a), 1);




                let parent_b = this.getBestRes(stack, "Object");
                ...
                let die_obj = this.getBestRes(stack,"Object");
            ...
            new_best_result = this.getBestRes(this.populations, "f(x)");
